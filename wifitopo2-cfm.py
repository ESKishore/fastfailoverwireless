...
def topobuild():
    ...
    # net.addController(c)             
        net.start()
        graph=nx.DiGraph()
        edges=[(0,1,1),(0,5,1),(1,5,1),(1,2,1),(2,6,1),(2,3,1),(2,4,1),(3,6,1),(3,4,1),(4,7,1),(5,6,1),(5,8,1),(5,10,1),(6,7,1),(6,8,1),(7,9,1),(8,9,1)]
        for (x,y,z) in edges:
            if (y,x,z) in edges:
                edges.remove((y,x,z))
...
def enable_CFM(net):
    """
     Connectivity Fault Management (CFM) is a network protocol used to detect link failure between two forwarding elements. 
    """
    ...
    for switch in switches:
        ...
        for intf in intfs:
            i+=1
            switch.cmd('ovs-vsctl set interface "%s" cfm_mpid="%d" other_config:cfm_interval=300 other_config:cfm_extended=true other_config:cfm_demand=true'%(intf, i))
